...
<body>
  <main class="shell">
    ...
    <section id="workspace" class="hidden">
      ...
      <div class="data-grid">
        ...
        <section class="panel"><div class="row"><h2>Локальный архив</h2><span id="archive-bytes" class="metric">—</span></div><p id="archive-meta" class="muted"></p><form id="archive-search-form" class="stack" style="margin:14px 0"><h3>Поиск по временной шкале</h3><p class="muted">Отбор происходит только по локальному индексу: сегменты, пересекающие выбранный интервал, остаются в выдаче.</p><select id="archive-search-camera" name="cameraId"><option value="">Все камеры</option></select><label class="muted">Начало <input name="from" type="datetime-local"></label><label class="muted">Окончание <input name="to" type="datetime-local"></label><button class="secondary">Найти сегменты</button></form><p id="archive-search-meta" class="muted"></p><ul id="archive" class="list"></ul><form id="export-form" class="stack" data-manager style="margin-top:16px"><div class="divider"></div><h3>Подготовка локального экспорта</h3><p class="muted">Создаётся манифест сегментов до 24 часов. Видео не отправляется в облако.</p><select id="export-camera" name="cameraId" required></select><label class="muted">Начало <input name="from" type="datetime-local" required></label><label class="muted">Окончание <input name="to" type="datetime-local" required></label><button>Подготовить манифест</button></form><ul id="exports" class="list" data-manager style="margin-top:16px"></ul><form id="retention-form" class="stack" data-manager style="margin-top:16px"><div class="divider"></div><h3>Очистка по политике хранения</h3><label class="muted">Удалить сегменты, завершившиеся раньше даты <input name="before" type="datetime-local" required></label><label class="muted">Ограничить архив, МиБ (необязательно) <input name="maxMib" type="number" min="0" step="1" placeholder="Например, 10240"></label><label class="checkbox"><input name="confirm" type="checkbox" required><span>Я понимаю, что подходящие файлы локального архива будут удалены без возможности восстановления.</span></label><button class="danger">Применить политику</button></form></section>
        <section class="panel"><h2>События и аналитика</h2><form id="analytics-search-form" class="stack" style="margin:12px 0"><p class="muted">Локальный поиск только нормализованных объектов: по камере, классу и времени.</p><select id="analytics-search-camera" name="cameraId"><option value="">Все камеры</option></select><input name="classification" placeholder="Класс, например person"><label class="muted">Начало <input name="from" type="datetime-local"></label><label class="muted">Окончание <input name="to" type="datetime-local"></label><div class="controls"><button class="secondary">Найти объекты</button><button type="reset" class="secondary">Показать весь журнал</button></div></form><p id="analytics-search-meta" class="muted"></p><ul id="events" class="list"></ul></section>
  ...
  <script>
    ...
    const message = $('#message'); let session; let cameras = []; let snapshots = []; let liveStreams = []; let policies = []; let healthChecks = []; let exportsData = []; let archiveSearchData = null; let analyticsSearchData = null;
    ...
    function renderArchiveSearchOptions() {
      const select = $('#archive-search-camera'); const selected = select.value;
      select.innerHTML = '<option value="">Все камеры</option>' + cameras.map(camera => '<option value="' + escapeHtml(camera.id) + '">' + escapeHtml(camera.name) + '</option>').join('');
      if ([...select.options].some(option => option.value === selected)) select.value = selected;
    }
    function renderArchive() {
      const result = archiveSearchData || { segments: window.archiveData.slice().reverse(), total: window.archiveData.length, bytes: window.storageData.bytes };
      $('#archive-search-meta').textContent = archiveSearchData ? 'Найдено сегментов: ' + result.total + '. Показано: ' + result.segments.length + '. Объём показанной выдачи: ' + formatBytes(result.bytes) + '.' : 'Показаны последние сегменты локального индекса.';
      show($('#archive'), result.segments, item => '<li><b>' + escapeHtml(cameras.find(camera => camera.id === item.cameraId)?.name || item.cameraId) + '</b><br><span class="muted">' + formatDate(item.startedAt) + ' — ' + formatDate(item.endedAt) + ' · ' + formatBytes(item.bytes || 0) + '</span></li>', 'Архив пока пуст');
    }
    function renderAnalyticsSearchOptions() {
      const select = $('#analytics-search-camera'); const selected = select.value;
      select.innerHTML = '<option value="">Все камеры</option>' + cameras.map(camera => '<option value="' + escapeHtml(camera.id) + '">' + escapeHtml(camera.name) + '</option>').join('');
      if ([...select.options].some(option => option.value === selected)) select.value = selected;
    }
    function renderEvents() {
      const result = analyticsSearchData || { events: window.eventsData, total: window.eventsData.length };
      $('#analytics-search-meta').textContent = analyticsSearchData ? 'Найдено аналитических событий: ' + result.total + '. Показано: ' + result.events.length + '.' : 'Показан весь локальный журнал событий.';
      show($('#events'), result.events, item => { const objects = item.type === 'analytics' ? (item.payload?.objects || []).map(object => escapeHtml(object.classification)).join(', ') : ''; return '<li><b>' + escapeHtml(item.type) + (objects ? ': ' + objects : '') + '</b><br><span class="muted">' + escapeHtml(cameras.find(camera => camera.id === item.cameraId)?.name || item.cameraId) + ' · ' + formatDate(item.occurredAt || item.at) + '</span></li>'; }, 'Событий пока нет');
    }
    async function load() {
      message.textContent = '';
      try {
        ...
        [cameras, window.recordingsData, liveStreams, window.archiveData, window.storageData, window.eventsData, snapshots, healthChecks] = core; archiveSearchData = null; analyticsSearchData = null;
        ...
        renderArchiveSearchOptions(); renderArchive(); renderAnalyticsSearchOptions(); renderEvents();
    ...
    $('#archive-search-form').onsubmit = async event => { event.preventDefault(); const form = new FormData(event.currentTarget); const params = new URLSearchParams(); for (const key of ['cameraId','from','to']) if (form.get(key)) params.set(key, form.get(key)); try { archiveSearchData = await api('/api/archive/search?' + params.toString()); renderArchive(); message.textContent = 'Поиск по архиву выполнен'; } catch (error) { message.textContent = error.message; } };
    $('#analytics-search-form').onsubmit = async event => { event.preventDefault(); const form = new FormData(event.currentTarget); const params = new URLSearchParams(); for (const key of ['cameraId','classification','from','to']) if (form.get(key)) params.set(key, form.get(key)); try { analyticsSearchData = await api('/api/analytics?' + params.toString()); renderEvents(); message.textContent = 'Поиск аналитических объектов выполнен'; } catch (error) { message.textContent = error.message; } };
    $('#analytics-search-form').onreset = () => { setTimeout(() => { analyticsSearchData = null; renderEvents(); }, 0); };
