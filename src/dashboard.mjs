...
<body>
  ...
  <script>
    ...
    const message = $('#message'); let session; let cameras = []; let snapshots = []; let liveStreams = []; let policies = []; let healthChecks = []; let exportsData = []; let archiveSearchData = null;
    ...
    async function load() {
      ...
      try {
        ...
        [cameras, window.recordingsData, liveStreams, window.archiveData, window.storageData, window.eventsData, snapshots, healthChecks] = core; archiveSearchData = null;
        ...
        renderArchiveSearchOptions(); renderArchive();
        ...
        show($('#events'), window.eventsData, item => { const objects = item.type === 'analytics' ? (item.payload?.objects || []).map(object => escapeHtml(object.classification)).join(', ') : ''; return '<li><b>' + escapeHtml(item.type) + (objects ? ': ' + objects : '') + '</b><br><span class="muted">' + escapeHtml(item.cameraId) + ' · ' + formatDate(item.occurredAt || item.at) + '</span></li>'; }, 'Событий пока нет');
