  ...
  <script>
    ...
    function renderCameras() {
      ...
      grid.innerHTML = cameras.map(camera => {
        ...
        return '<article class="camera-card"><div class="thumb">' + media + '<span class="status">' + escapeHtml(health?.state || camera.status) + '</span></div><div class="camera-body"><h3>' + escapeHtml(camera.name) + '</h3><p class="muted">' + escapeHtml(camera.mode.toUpperCase()) + ' · ' + escapeHtml(camera.address) + '</p>' + profiles + (health ? '<p class="muted">Проверка: ' + escapeHtml(health.detail || health.state) + ' · ' + formatDate(health.checkedAt) + '</p>' : '') + (live ? '<p class="muted">HLS: запущен ' + formatDate(live.startedAt) + '</p>' : '') + (snapshot ? '<p class="muted">Снимок: ' + formatDate(snapshot.capturedAt) + '</p>' : '') + controls + '</div></article>';
      }).join('');
    }
